...
class face_utils_cls(object):
    def __init__(self):
        ...
        dlib库对应的关键点模型
        '''
        self.face_landmark_dlib = dlib.shape_predictor('models/shape_predictor_68_face_landmarks.dat')
        ...
        self.face_feature_sess = tf.Session(graph=tf.Graph(), config=session_config)
        self.face_detection_sess = tf.Session(graph=tf.Graph(), config=session_config)
        self.face_landmark_sess = tf.Session(graph=tf.Graph(), config=session_config)
        self.face_attribute_sess = tf.Session(graph=tf.Graph(), config=session_config)
    ...
    def init_feature_face(self):
        with self.face_feature_sess.as_default():
            with self.face_feature_sess.graph.as_default():
                with tf.gfile.FastGFile(self.ff_pb_path, 'rb') as f:
    ...
    def init_detection_face_tf(self):
        with self.face_detection_sess.as_default():
            with self.face_detection_sess.graph.as_default():
                ...
                with tf.gfile.GFile(self.detect_pb_path, 'rb') as fid:
                    ...
                    for key in ['num_detections', 'detection_boxes', 'detection_scores','detection_classes']:
                        ...
                        if tensor_name in all_tensor_names:
                            self.detection_tensor_dict[key] = tf.get_default_graph().get_tensor_by_name(
                                tensor_name)
                    self.detection_image_tensor = tf.get_default_graph().get_tensor_by_name('image_tensor:0')
    ...
    def init_face_landmark_tf(self):
        ...
        with self.face_landmark_sess.as_default():
            with self.face_landmark_sess.graph.as_default():
                ...
                with tf.gfile.GFile(self.landmark_pb_path, 'rb') as fid:
                    serialized_graph = fid.read()
                    graph_def.ParseFromString(serialized_graph)
                    tf.import_graph_def(graph_def, name='')
                    self.face_landmark_tensor = tf.get_default_graph(). \
    ...
    def init_face_attribute(self):
        ...
        with self.face_attribute_sess.as_default():
            with self.face_attribute_sess.graph.as_default():
                ...
                with tf.gfile.GFile(self.attribute_pb_path, 'rb') as fid:
                    serialized_graph = fid.read()
                    graph_def.ParseFromString(serialized_graph)
                    tf.import_graph_def(graph_def, name='')
                    self.pred_eyeglasses = tf.get_default_graph().get_tensor_by_name("ArgMax:0")
                    self.pred_young = tf.get_default_graph().get_tensor_by_name("ArgMax_1:0")
                    self.pred_male = tf.get_default_graph().get_tensor_by_name("ArgMax_2:0")
                    self.pred_smiling = tf.get_default_graph().get_tensor_by_name("ArgMax_3:0")
                    self.face_attribute_image_tensor = tf.get_default_graph().get_tensor_by_name("Placeholder:0")
...
if __name__ == '__main__':
    ...
    im_data = cv2.imread("/home/dell/workspace/mc_code/Flask_code/flask_server_facerecognition/tmp/tmp.jpg")
